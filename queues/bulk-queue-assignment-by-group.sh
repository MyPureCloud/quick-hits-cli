# >> START cli-assign-to-queue-by-group A set of Genesys Cloud CLI commands the will identify a user by group and then add those users to a queue.

#######
#   Bulk assignment to queue by group
#  
#  This files show the list of Genesys Cloud CLI commands needed to find all the users belonging to a particular group and move those users to a queue.
...
do
   ...
   # Chunk out 100 records from the groups-user.json file and create a set of files by batch to add the users to the queue. 
...
# >> END cli-assign-to-queue-by-group
