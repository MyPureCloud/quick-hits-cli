...
Write-OutPut '{"name": "Barbie_Queue"}' | gc.exe queues create | ConvertFrom-Json | Select-Object id
...
gc.exe -p SOURCE_OAUTH_CLIENT queues list -a > queues.json                  # Dump all of the queues out of the target org into a file
...
Get-ChildItem -Path ".\temp\*" -Include queue-*.json -Name |                # Take all the file names and use the cli to create the queues
    ForEach-Object { gc.exe -p DEST_OAUTH_CLIENT queues create -f $_ }
