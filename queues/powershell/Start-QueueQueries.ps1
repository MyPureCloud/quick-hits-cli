...
(gc.exe queues list -a | ConvertFrom-Json) | 
...
(gc.exe users list -a | ConvertFrom-Json) |
    ...
    ForEach-Object { (gc.exe users queues get $_.id -a | ConvertFrom-Json) | Write-Output } | 
...
(gc.exe queues list -a | ConvertFrom-Json) | 
    ...
    Select-Object id | 
    ForEach-Object { 
        (gc.exe queues users get $_.id -a | ConvertFrom-Json) | 
