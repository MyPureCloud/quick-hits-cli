...
gc --p SOURCE_OAUTH_CLIENT queues list > queues.json                                                       # Dump all of the queues out of the target org into a file
...
ls /tmp/queues-*.json  | xargs -I{} gc -p DEST_OAUTH_CLIENT queues create -f {}                            # Take all the file names and use the cli to create the queue
