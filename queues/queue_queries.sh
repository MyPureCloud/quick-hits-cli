...
gc routing queues list -a |jq -c '.[] | select( .name | contains("jcc-chat6"))' | jq -r '. | .id' | xargs -I{} gc routing queues members list {} |jq -n '[inputs]'| jq -r '.[] | map({"id": .user.id, "username": .user.username})' | jq -r '(.[0] | keys_unsorted) as $keys | ([$keys] + map([.[ $keys[] ]])) [] | @csv'
