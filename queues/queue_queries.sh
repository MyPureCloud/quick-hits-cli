...
gc users list -a | jq -c '.[] | select( .username | contains("user24927@testme.test"))' |jq -r '. | .id' | xargs -I{} gc users queues list -a {} | jq -r '(.[0] | keys_unsorted) as $keys | ([$keys] + map([.[ $keys[] ]])) [] | @csv'
...
gc routing queues list -a |jq -c '.[] | select( .name | contains("jcc-chat6"))' | jq -r '. | .id' | xargs -I{} gc routing queues members list -a {} |jq -n '[inputs]'| jq -r '.[] | map({"id": .user.id, "username": .user.username})' | jq -r '(.[0] | keys_unsorted) as $keys | ([$keys] + map([.[ $keys[] ]])) [] | @csv'
