...
gc users list --pageSize=100 --expand="authorization" | jq -c '.[] | select( .authorization.roles[].name =="Supervisor")' | -r jq .id > authorizations-user.json
 
# Get a list of all users by role and convert the data to a csv file
gc users list --pageSize=100 --expand="authorization" | jq -c '.[] | select( .authorization.roles[].name =="Supervisor")' |jq -n '[inputs]'|  jq -r '. | map({"id": .id, "division": .division.name, "email": .email})' | jq -r '(.[0] | keys_unsorted) as $keys | ([$keys] + map([.[ $keys[] ]])) [] | @csv'

# Get a list of all of the user ids with their skill information with their skill information
gc users list --pageSize=100 --expand="skills" | jq -c '.[] |  select( .skills[].name == "AcdSkill9")' | jq -r .id > skills-user.json
